<script lang="ts">
  export let data: { backend: Record<string, unknown> | null; error: string | null };
</script>
...
{:else if data.backend}
  <pre class="text-sm bg-slate-50 border border-slate-200 rounded p-4 overflow-auto">{JSON.stringify(data.backend, null, 2)}</pre>
{:else}
