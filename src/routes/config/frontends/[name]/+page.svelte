<script lang="ts">
  ...
  async function removeBind(bindName: string) {
    if (!data.frontend?.name || !confirm(`Bind „${bindName}“ entfernen?`)) return;
    try {
      const res = await fetch(
...
<h1 class="text-2xl font-semibold mb-2">Frontend: {data.frontend?.name ?? data.error ?? '—'}</h1>
...
{:else if data.frontend}
  <section class="mb-6">
    ...
    <div class="flex flex-wrap items-end gap-3">
      <label class="block">
        ...
        <input type="text" value={data.frontend.name} disabled class="mt-1 block w-48 rounded border border-slate-300 bg-slate-100 px-2 py-1.5 text-sm" />
      </label>
      <label class="block">
        <span class="text-sm text-slate-600">Backend</span>
        <select bind:value={defaultBackend} class="mt-1 block rounded border border-slate-300 px-2 py-1.5 text-sm bg-white">
  ...
  <section class="mb-6">
      ...
      <ul class="border border-slate-200 rounded divide-y divide-slate-200 mb-3">
        {#each data.binds as bind}
          ...
          <li class="flex items-center justify-between px-3 py-2 text-sm">
            <span>{bindName}</span>
            <span class="text-slate-500">{bind.address ?? '*'}:{bind.port ?? ''}</span>
            <button
              type="button"
    ...
    {:else}
      <p class="text-slate-500 text-sm mb-2">Keine Binds. Mindestens einen hinzufügen (Adresse:Port).</p>
    {/if}
    <div class="flex flex-wrap gap-2 items-end">
      <input type="text" bind:value={addBindName} placeholder="Name (optional)" class="w-32 rounded border border-slate-300 px-2 py-1.5 text-sm" />
      <input type="text" bind:value={addBindAddress} placeholder="Adresse" class="w-32 rounded border border-slate-300 px-2 py-1.5 text-sm" />
      <input type="number" bind:value={addBindPort} min="1" max="65535" class="w-24 rounded border border-slate-300 px-2 py-1.5 text-sm" />
      <button
        type="button"
